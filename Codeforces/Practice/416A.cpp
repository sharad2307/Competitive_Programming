...
int main()
{
	...
	string s;
	ll a;
	ll low=-1000000001,high=1000000001;
	char c;
	while(n--)
	{
		cin>>s;
		...
		cin>>c;
		// cout<<a<<nl;
		if( (s==">=" && c=='Y')) low=max(low,a);
		else if(s=="<=" && c=='Y') high=min(high,a);
		else if( (s==">" && c=='Y') || (s=="<=" && c=='N') ) low=max(low,a+1);
		else if( (s=="<" && c=='Y') || (s==">=" && c=='N')) high=min(high,a-1);
		else if(s==">" && c=='N')high=min(high,a);
		else if( (s=="<" && c=='N'))low=max(low,a);
